...
const PageDashboardPayments = async () => {
  // Consultar pagos de los últimos 30 días
  ...
  return (
    <section className="p-2 overflow-x-auto">
      <DashboardHeader title="Gestión de Pagos (Mercado pago)" />

      <table className="min-w-full bg-white border border-gray-200 text-sm">
        <thead className="bg-gray-100">
          <tr>
            <th className="border border-gray-200 px-4 py-2">Email del Pagador</th>
          ...
          Identificación del Pagador
            ...
            <th className="border border-gray-200 px-4 py-2">Monto Transacción</th>
            <th className="border border-gray-200 px-4 py-2">Monto Neto Recibido</th>
            ...
            <th className="border border-gray-200 px-4 py-2">Fecha de Creación</th>
            <th className="border border-gray-200 px-4 py-2">Fecha de Aprobación</th>
            <th className="border border-gray-200 px-4 py-2">Última Actualización</th>
            <th className="border border-gray-200 px-4 py-2">Estado</th>
            <th className="border border-gray-200 px-4 py-2">Estado Detallado</th>
            {/* <th className="border border-gray-200 px-4 py-2">
              Referencia Externa
            </th> */}
            ...
            <th className="border border-gray-200 px-4 py-2">Fecha de Expiración</th>
            <th className="border border-gray-200 px-4 py-2">Fecha de Liberación del Dinero</th>
          </tr>
        </thead>
        <tbody>
          {pagos.results && pagos.results.length > 0 ? (
            pagos.results.map((pago, index) => (
              <tr key={index} className={index % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                <td className="border border-gray-200 px-4 py-2">{pago.payer?.email}</td>
                ...
                <td className="border border-gray-200 px-4 py-2">{pago.description}</td>
                <td className="border border-gray-200 px-4 py-2">${pago.transaction_amount}</td>
                <td className="border border-gray-200 px-4 py-2">
                  ${pago.transaction_details?.net_received_amount}
                </td>
                <td className="border border-gray-200 px-4 py-2">{pago.payment_method_id}</td>
                <td className="border border-gray-200 px-4 py-2">{pago.payment_type_id}</td>
                <td className="border border-gray-200 px-4 py-2">{pago.id}</td>
                <td className="border border-gray-200 px-4 py-2">
                  {pago.date_created ? new Date(pago.date_created).toLocaleString() : "N/A"}
                </td>
                <td className="border border-gray-200 px-4 py-2">
                  {pago.date_approved ? new Date(pago.date_approved).toLocaleString() : "N/A"}
                </td>
                <td className="border border-gray-200 px-4 py-2">
                ...
                <td className="border border-gray-200 px-4 py-2">{pago.status}</td>
                <td className="border border-gray-200 px-4 py-2">{pago.status_detail}</td>
                {/* <td className="border border-gray-200 px-4 py-2">
                ...
                <td className="border border-gray-200 px-4 py-2">{pago.currency_id}</td>
                {/* <td className="border border-gray-200 px-4 py-2">
                  {pago.live_mode ? "Sí" : "No"}
                </td> */}
                <td className="border border-gray-200 px-4 py-2">{pago.installments}</td>
                {/* <td className="border border-gray-200 px-4 py-2">
            ...
            <tr>
              <td colSpan={18} className="text-center py-8 text-gray-500">
                No se encontraron pagos en fechas recientes
              </td>
            </tr>
          )}
        </tbody>
