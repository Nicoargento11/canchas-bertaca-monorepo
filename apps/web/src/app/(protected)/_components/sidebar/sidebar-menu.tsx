"use client";
import { ArrowRightFromLine, ArrowLeftToLine, UserCircle2, MoreVertical } from "lucide-react";
...
export const SidebarMenu: React.FC<SideBarProps> = ({ children, user, slug }) => {
  ...
  return (
    ...
    >
      <nav className="h-full w-fit flex flex-col bg-white border-r border-gray-100 shadow-sm fixed z-20">
        <div className="p-4 pb-2 flex justify-between items-center">
          <div className={`overflow-hidden transition-all text-Primary ${!expanded && "w-0"}`}>
            <Logo slug={slug} />
          </div>
          <Button
            onClick={() => {
