"use client";
import getReserveById from "@/actions/reserve/get-reserve-id";
...
export default function PaymentFailure() {
  ...
  const [reserve, setReserve] = useState<ReservesWithUser | null>();

  const reserveId = searchParams.get("external_reference");

  useEffect(() => {
    if (reserveId) {
      getReserveById(reserveId).then((reserve) => setReserve(reserve));
    }
  }, []);
  return (
      ...
      <div className="max-w-md p-6 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <div className="flex flex-col gap-2 items-center">
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-400">
            Lo sentimos, no pudimos procesar tu reserva. Por favor, intenta de
            nuevo o más tarde.
          </p>
