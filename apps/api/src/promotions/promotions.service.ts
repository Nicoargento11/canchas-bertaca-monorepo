...
export class PromotionsService {
    ...
    async create(createPromotionDto: CreatePromotionDto) {
        ...
        if (createPromotionDto.type === PromotionType.GIFT_PRODUCT) {
            const hasGiftProducts = createPromotionDto.giftProducts && createPromotionDto.giftProducts.length > 0;
            ...
            if (hasGiftProducts) {
                const productIds = createPromotionDto.giftProducts.map(gp => gp.productId);
                const products = await this.prisma.product.findMany({
                    where: { id: { in: productIds } },
                });
                if (products.length !== productIds.length) {
                    throw new NotFoundException('Uno o más productos a regalar no fueron encontrados');
                }
            }
        ...
        if (
            ...
            (createPromotionDto.value === undefined || createPromotionDto.value === null)
        ) {
            throw new BadRequestException(
    ...
    async update(id: string, updatePromotionDto: UpdatePromotionDto) {
        ...
        if (updatePromotionDto.giftProducts && updatePromotionDto.giftProducts.length > 0) {
            const productIds = updatePromotionDto.giftProducts.map(gp => gp.productId);
            const products = await this.prisma.product.findMany({
                where: { id: { in: productIds } },
            });
            if (products.length !== productIds.length) {
            ...
            return await this.prisma.promotion.update({
                ...
                data: {
                    ...
                    validFrom: restDto.validFrom
                        ...
                        : undefined,
                    validTo: restDto.validTo
                        ? new Date(restDto.validTo)
                        : undefined,
                },
                include: {
                    complex: { select: { id: true, name: true } },
    ...
    async validatePromotion(
        ...
        if (promotion.complexId !== complexId) {
            return { valid: false, reason: 'La promoción no aplica para este complejo' };
        }

        // Verificar tipo de deporte
        if (promotion.sportTypeId && promotion.sportTypeId !== sportTypeId) {
            return { valid: false, reason: 'La promoción no aplica para este deporte' };
        }

        // Verificar cancha
        if (promotion.courtId && promotion.courtId !== courtId) {
            return { valid: false, reason: 'La promoción no aplica para esta cancha' };
        }
        ...
        if (promotion.daysOfWeek.length > 0 && !promotion.daysOfWeek.includes(dayOfWeek)) {
            return { valid: false, reason: 'La promoción no aplica para este día de la semana' };
        }

        // Verificar horario
        if (promotion.startTime && promotion.endTime) {
            const [promoStartH, promoStartM] = promotion.startTime.split(':').map(Number);
            ...
            if (reserveTime < promoStart || reserveTime >= promoEnd) {
                return { valid: false, reason: 'La promoción no aplica para este horario' };
            }
        }
